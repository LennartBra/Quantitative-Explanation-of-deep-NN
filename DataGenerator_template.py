...
class DataGenerator(Sequence):
    def __init__(self, path_main, list_id, batch_size, typ="big", n_sample=1000, n_classes=2, shuffle=True):        
        ...
        ##        
        self._nr_batches = 0 
        ## @brief Last index of segments of actual subject.
        ##
        self.inPatientIdx = 0
    ...
    def __count_batches(self):
        ##
        # @brief    This method counts the total number of batches.
        ...
        for i in range(len(self.list_id)):
            dev0_abp = np.load(self.path_main+"dev0_abp/"+self.list_id[i], allow_pickle=True)
    ...
    def __data_generation(self):
        ...
        for i in range(self.batch_size):
            ...
            #print(f'inPatientIdx:{self.inPatientIdx},n_classes:{self.n_classes},y:{y.shape},_target:{self._target.shape},ID:{self.list_id[self._id_idx]}')
    ...
    # Method --> Load data of one patient
    def __load_data(self):
        ...
        if len(self._dev0) == 1:
            self._target = np.expand_dims(self._target,1).T

            

class DataGenerator_Pressure(Sequence):
    def __init__(self, path_main, list_id, batch_size, typ="big", n_sample=1000, n_classes=2, shuffle=False):        
        ##
        # @brief This constructor initalizes the DataGenerator object.
        # @param path_main      The main path which includes the preprocessed data.
        # @param list_id        The list of the subject IDs.                 
        ...
        ## @brief Input data of actual subject.
        ##
        self._dev0_abp = np.load(self.path_main+"dev0_abp/"+self.list_id[0], allow_pickle=True)
        ## @brief First derivation of input data of actual subject.
        ##
        self._dev1_abp = np.load(self.path_main+"dev1_abp/"+self.list_id[0], allow_pickle=True)
        ## @brief Second derivation of input data of actual subject.
        ##
        self._dev2_abp = np.load(self.path_main+"dev2_abp/"+self.list_id[0], allow_pickle=True)
        ## @brief Target data of actual subject.
        ##
        self._target = np.load(self.path_main+"ground_truth/"+self.list_id[0])
    ...
    def __data_generation(self):
        ...
        for i in range(self.batch_size):
            ...
            if self.inPatientIdx==len(self._dev0_abp):
                if self._id_idx==len(self.list_id)-1:
                    break
                else:
                    self._id_idx += 1
            ...
            x1[i] = self._dev0_abp[self.inPatientIdx]
            x2[i] = self._dev1_abp[self.inPatientIdx]
            x3[i] = self._dev2_abp[self.inPatientIdx]
            #print(f'inPatientIdx:{self.inPatientIdx},n_classes:{self.n_classes},y:{y.shape},_target:{self._target.shape},ID:{self.list_id[self._id_idx]}')
            y[i] = self._target[self.inPatientIdx, :self.n_classes]
            self.inPatientIdx += 1
    ...
    def __load_data(self):
        ...
        # @brief This method loads data of the next subject.
        ##   
        self._dev0_abp = np.load(self.path_main+"dev0/"+self.list_id[self._id_idx], allow_pickle=True)
        self._dev1_abp = np.load(self.path_main+"dev1/"+self.list_id[self._id_idx], allow_pickle=True)
        self._dev2_abp = np.load(self.path_main+"dev2/"+self.list_id[self._id_idx], allow_pickle=True)
        self._target = np.load(self.path_main+"ground_truth/"+self.list_id[self._id_idx])
        
        if len(self._dev0_abp) == 1:
            self._target = np.expand_dims(self._target,1).T



class DataGenerator_Pressure_onesignal(Sequence):
    def __init__(self, path_main, list_id, batch_size, typ="big", n_sample=1000, n_classes=2, shuffle=True):        
        ##
        # @brief This constructor initalizes the DataGenerator object.
        # @param path_main      The main path which includes the preprocessed data.
